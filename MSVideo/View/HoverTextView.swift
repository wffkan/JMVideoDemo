...
//        textView.setValue(placeHolderLabel, forKey: "_placeholderLabel")
        ...
        sendImageView.frame = CGRect.init(x: UIScreen.width, y: 0, width: 50, height: 50)
        ...
        splitLine = UIView.init(frame: CGRect.init(x: 0, y: 0, width:UIScreen.width, height: 0.5))
    ...
    func updateTextViewFrame() {
        let textViewHeight = keyboardHeight > UIScreen.safeAreaBottomHeight ? textHeight + 2*TOP_BOTTOM_INSET : (textView.font?.lineHeight ?? 0) + 2*TOP_BOTTOM_INSET
        self.textView.frame = CGRect.init(x: 0, y: UIScreen.height - keyboardHeight - textViewHeight, width:UIScreen.width, height: textViewHeight)
    }
    
    func updateRightViewsFrame() {
        var originX = UIScreen.width
        originX -= keyboardHeight > UIScreen.safeAreaBottomHeight ? 50 : (textView.text.count > 0 ? 50 : 0)
    ...
    func updateIconState() {
        editImageView.image = keyboardHeight > UIScreen.safeAreaBottomHeight ? UIImage.init(named: "ic90Pen1") : (textView.text.count > 0 ? UIImage.init(named: "ic90Pen1") : UIImage.init(named: "ic30Pen1"))
        atImageView.image = keyboardHeight > UIScreen.safeAreaBottomHeight ? UIImage.init(named: "ic90WhiteAt") : (textView.text.count > 0 ? UIImage.init(named: "ic90WhiteAt") : UIImage.init(named: "ic30WhiteAt"))
...
extension HoverTextView:UITextViewDelegate {
    ...
    func textViewDidChange(_ textView: UITextView) {
        ...
        if !textView.hasText {
            ...
            textHeight = attributedText.multiLineSize(width: UIScreen.width - LEFT_INSET - RIGHT_INSET).height
        }
        updateViewFrameAndState()
    }
